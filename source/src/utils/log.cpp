...
void Log::Init()
{
    s_SpdLogger = spdlog::stdout_color_mt<spdlog::async_factory>("LearnVulkan");
    s_SpdLogger->set_level(spdlog::level::trace);
    s_SpdLogger->set_pattern("%^%H:%M:%S:%e [%P-%t] [%1!L] [%20s:%-4#] - %v%$");
}

void Log::Logger(spdlog::source_loc loc, spdlog::level::level_enum lvl, const spdlog::memory_buf_t &buffer)
{
